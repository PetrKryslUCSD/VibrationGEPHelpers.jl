let
    ...
    d, v, nconv = gep_smallest(K + omega_shift^2 * M, M, neigvs; method = :KrylovKit, orthogonalize = true, maxiter=500)
    d .-= omega_shift^2
    @test nconv == neigvs
    fs1 = sqrt.(abs.(d)) ./ (2*pi)
    ...
    d, v, nconv = gep_smallest(K + omega_shift^2 * M, M, neigvs; method = :KrylovKit, orthogonalize = true)
    d .-= omega_shift^2
    @test nconv == neigvs
    fs1 = sqrt.(abs.(d)) ./ (2*pi)
    @test norm(fs1 - fs) / norm(fs) <= 1e-5
