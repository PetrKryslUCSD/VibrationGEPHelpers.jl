...
- `max_vMv_diag_error`, `max_vMv_offdiag_error`, `max_i`, `max_j`: absolute deviations of the
  diagonal entries of the reduced mass matrix from unity, absolute deviations of
  the off-diagonal entries of the reduced mass matrix from zero, and the row and
  column index of the entry with the largest error.
"""
function check_M_orthogonality(v, M)
    ...
    return max_vMv_diag_error, max_vMv_offdiag_error, max_i, max_j
end
...
- `max_vKv_diag_error`, `max_vKv_offdiag_error`, `max_i`, `max_j`: absolute deviations of the
  ...
  matrix from zero, and the row and column index of the entry with the largest error.
...
function check_K_orthogonality(d, v, K)
    ...
    return max_vKv_diag_error, max_vKv_offdiag_error, max_i, max_j
end
